...
describe('EmptyState', () => {
  ...
  it('defaults the heading to h3 so it never outranks a page/section h2', () => {
    ...
    expect(screen.getByRole('heading', { level: 3, name: 'No projects yet' })).toBeDefined();
  });

  it('headingAs="h2" is honoured for a standalone empty state', () => {
    render(<EmptyState icon="info" title="No projects yet" headingAs="h2" />);
    expect(screen.getByRole('heading', { level: 2, name: 'No projects yet' })).toBeDefined();
  });

  it('renders the one action when given, keeping the state actionable rather than a dead end', () => {
    render(<EmptyState icon="info" title="No projects yet" action={<a href="/book-audit">Book a design audit</a>} />);
    expect(screen.getByRole('link', { name: 'Book a design audit' })).toBeDefined();
  });
});
