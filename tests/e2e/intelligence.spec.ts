...
test('the hub links to every feature, and the nav links to the hub', async ({
  page,
}) => {
  await page.goto('/');
  await page
...
for (const feature of FEATURES) {
  test(`${feature.slug} renders as its own page with one h1`, async ({
    page,
  }) => {
...
test('Vastu-Tech shows the human-in-the-loop promise, not just the AI claim', async ({
  page,
}) => {
  ...
  await expect(
    page.getByText(/The AI narrows the question; a person answers it/),
  ).toBeVisible();
});
...
test('the how-it-works stepper is real text, not an image', async ({
  page,
}) => {
...
test('features with a scene reserve its slot; the one without does not', async ({
  page,
}) => {
...
test('an unknown capability is a 404, not a blank page', async ({
  request,
}) => {
