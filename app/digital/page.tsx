  ...
  const packages = [
    {
      ...
      price: '₹25,000 / room',
      summary: 'Essential remote design package for homeowners who want professional layout guidance and color palettes.',
    ...
    },
    {
      ...
      price: '₹45,000 / room',
      recommended: true,
      summary: 'Complete architectural design package with 4K 3D renders and detailed working drawings for contractor execution.',
      href: '/digital/pro',
      inclusions: [
    ...
    },
    {
      ...
      price: '₹75,000 / room',
      summary: 'Ultra-luxury remote design with 360 VR walkthrough, physical material sample box shipped to your door, and 3 video calls.',
    ...
    <main id="main" tabIndex={-1}>
      <JsonLd
        data={{
          ...
          description: 'Complete, professional architectural interior design delivered remotely.',
          url: ROUTE,
        }}
      />
        ...
        <Container>
          ...
          <Stack gap={6} className="max-w-4xl">
            ...
            <p className="text-[length:var(--typography-body-lg-font-size)] text-on-surface-2 font-medium leading-relaxed max-w-3xl">
              Not everyone needs studio-managed execution. Luxe Axis Digital gives you complete, professional architectural design — 2D layouts, 4K 3D VR renders, Vastu-Tech compass scans, and an itemized shop-it-yourself list — remotely, affordably, and on your schedule.
            </p>
          ...
          {packages.map((pkg) => (
            <div
              ...
              className={`lx-liquid-glass rounded-2xl p-6 border flex flex-col justify-between relative ${
                pkg.recommended ? 'border-accent shadow-[0_0_30px_rgba(212,175,55,0.15)] bg-accent/5' : 'border-accent/30'
              }`}
            >
              {pkg.recommended && (
                <div className="absolute -top-3 left-6">
                  <Badge tone="accent" icon="check">Most Popular</Badge>
                </div>
              )}
              <Stack gap={4}>
                <div>
                  <h3 className="font-display text-h3 font-bold text-on-surface">{pkg.name}</h3>
                  <strong className="block font-display text-h2 text-accent font-bold mt-1">{pkg.price}</strong>
                  <p className="text-small text-on-surface-2 mt-2 leading-relaxed">{pkg.summary}</p>
                </div>
              ...
              <div className="pt-6">
                <Button as="a" href={pkg.href} variant={pkg.recommended ? 'primary' : 'secondary'} className="w-full justify-center">
                  Explore {pkg.name} →
                </Button>
              </div>
            </div>
        ...
        <div className="lx-liquid-glass rounded-2xl p-8 border border-accent/30">
          <div className="max-w-3xl mb-8">
            ...
            <p className="text-body text-on-surface-2 leading-relaxed">
              Design Club is our private community for design enthusiasts, homeowners, and real-estate investors. Members receive exclusive trade pricing on premium fittings, priority studio appointments, and early access to trend lookbooks.
            </p>
          </div>

          <Grid cols={2} gap={6}>
            {clubBenefits.map((b) => (
              <div key={b.title} className="p-5 rounded-xl bg-surface-deep/40 border border-accent/20">
                <h4 className="font-display text-h4 font-bold text-accent mb-1">{b.title}</h4>
                <p className="text-small text-on-surface-2 leading-relaxed">{b.desc}</p>
              </div>
            ))}
          </Grid>
        </div>
          ...
          <table className="w-full text-left text-small">
            <thead>
              <tr className="border-b border-border-subtle/60 text-accent font-display text-body font-bold">
                ...
                <th className="py-3 px-4 text-accent bg-accent/10 rounded-t-lg">Pro Package</th>
