    ...
    platforms: {
      css: {
        ...
        files: [{
          ...
          // NO file-level filter. Filtering a tier that other tokens alias
          // makes getReferences resolve via its unfilteredTokens fallback and
          // log "filtered out token references were found" on every build.
          // The only blunt instrument for that is log.warnings:'disabled',
          // which also silences name-collision and font-shorthand warnings —
          // both live risks here. Emit everything; narrow textually below.
          options: {
            selector: mode.selector,
            // Component tokens keep their var() reference, so flipping
            // data-theme re-resolves them with no rebuild. Semantic tokens
            // resolve to literals — they alias theme.*, which is dropped from
            // the final output, so an unresolved reference would point at a
...
// Both intermediate files are unfiltered, so each contains every tier.
// Rebuild the final blocks textually, keeping only what belongs in each.
// A css/variables file has exactly one rule, so the first `{` and last `}`
// bound the whole declaration list.
...
const declName = (line: string): string | null => line.match(/--([a-z0-9-]+):/i)?.[1] ?? null;

// Role names come from the mode file, not a hardcoded list, so they cannot
// drift from the tokens themselves.
const semanticRoles = new Set(
  Object.keys(darkMode.semantic).filter((key) => !key.startsWith('$')),
);

// dark → :root keeps everything except the internal theme tier.
// light → [data-theme] keeps only the semantic overrides; everything else is
// theme-independent and would otherwise be declared twice.
const darkBlock = [
  ':root {',
  ...declarations(readFileSync('styles/tokens.dark.css', 'utf8')).filter((l) => !/--theme-/.test(l)),
  '}',
].join('\n');

const lightBlock = [
  '[data-theme="light"] {',
  ...declarations(readFileSync('styles/tokens.light.css', 'utf8')).filter((l) => {
...
Two mechanical notes. `tsx` loads a bare `.ts` entrypoint as CJS, and esbuild rejects top-level `await` under CJS output — wrap the body in an `async function main()` and call it. And import `darkMode` from `tokens/modes/dark.json` alongside the `tokens` import.

The `--theme-` prefix is what the `name/luxe-css` transform produces for `theme.*` tokens; verify against the real generated file rather than assuming.
...
- **Language switcher (EN / தமிழ்)** — ✅ **DONE, after this plan.** `components/LangSwitch.tsx` shipped in `df86b0d` (Header + MobileSheet), and the persistence half — `middleware.ts` honouring the app-owned `lx-locale` cookie written by that switch, gated on `isPublished` so it cannot cycle against the `/ta` → English redirect — landed later still. Tamil discovery no longer depends on typing the URL, and the trade-off recorded below is no longer being accepted. The paragraph is kept verbatim as the historical record of why the component is shaped the way it is; `i18n/routing.ts` and `middleware.ts` carry the current, accurate account. Original text follows: spec §2.3 and Design System §3.3 both require a "Language switch … persists via cookie" utility, and none exists yet (confirmed: grepping app/, components/, features/, lib/ for `/ta` returns only lib/seo/hreflang.ts, which is metadata, not a link). Commit 52ecaa2 set `i18n/routing.ts`'s `localeDetection: false` to kill an infinite redirect loop; the accepted trade-off documented there is that a Tamil-preferring visitor gets no automatic nudge to `/ta` even at the published `/` and must type the URL themselves — that was the only working discovery path before this fix, and it's now gone with nothing replacing it. This switcher is now mandatory, not optional polish, to restore Tamil discoverability. Constraints for whoever builds it, per the comment in `i18n/routing.ts`: it must navigate explicitly to `/ta<route>`, must not rely on next-intl's locale detection, and cannot use `NEXT_LOCALE` for persistence — with `localeDetection: false`, next-intl ignores that cookie entirely (read and write), so the spec's "persists via cookie" needs the switcher's own app-owned cookie. If the no-auto-detection UX ever proves unacceptable in the meantime, the fix is NOT re-enabling `localeDetection` (that reintroduces the loop on every unpublished route) — it is moving detection into `middleware.ts`, which already imports `isPublished`, and auto-redirecting to `/ta<route>` only when that route is published.
