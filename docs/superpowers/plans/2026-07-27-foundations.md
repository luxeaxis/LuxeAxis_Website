...
// Concatenate, then append the nav glass recipe with its fallback (spec §2.2).
const dark = readFileSync('styles/tokens.dark.css', 'utf8');
const light = readFileSync('styles/tokens.light.css', 'utf8');
const glass = `
:root {
  --nav-bg: rgba(13, 43, 78, 0.55);
  --nav-bg: color-mix(in srgb, var(--surface) 55%, transparent);
}
[data-theme="light"] {
  --nav-bg: rgba(252, 250, 245, 0.7);
...
The duplicated `--nav-bg` declaration is the fallback pattern: a browser without `color-mix` keeps the first, one with support takes the second. Without it, unsupported browsers get a transparent nav rather than dark glass.
...
describe('token build', () => {
  ...
  it('ships a static fallback before every color-mix declaration', () => {
    const navDeclarations = css.match(/--nav-bg:[^;]+;/g) ?? [];
    expect(navDeclarations).toHaveLength(4);
    expect(navDeclarations[0]).toContain('rgba(');
    expect(navDeclarations[1]).toContain('color-mix');
  });
